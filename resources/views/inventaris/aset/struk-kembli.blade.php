...
<head>
    <meta charset="UTF-8">
    <title>Bukti Pengembalian Aset - {{ $pemakai->nomor_pengembalian }}</title>
    <style>
        ...
        .badge { display: inline-block; margin-top: 8px; padding: 3px 10px; border-radius: 999px; font-size: 11px; font-weight: bold; }
...
<body>
    ...
    <div class="judul" style="max-width:380px;margin:0 auto;">
        Bukti Pengembalian Aset - {{ $pemakai->nomor_pengembalian }}
    </div>

    <div class="kwitansi">
        <div class="center">
            <div class="brand">Marimas One</div>
            <div class="brand">Bukti Pengembalian Aset</div>
            <div class="nomor">{{ $pemakai->nomor_pengembalian }}</div>
            <div class="tanggal">{{ $pemakai->tanggal_kembali->translatedFormat('d M Y') }}</div>
        </div>
        ...
        <div class="row">
            <span class="label">Dikembalikan Oleh</span>
            <span class="value">{{ $pemakai->penerima->name }}</span>
        </div>
        <div class="row">
            <span class="label">Ref. Serah Terima</span>
            <span class="value">{{ $pemakai->nomor_serah_terima }}</span>
        </div>

        <div class="center">
            <span class="badge {{ $pemakai->aset->status === 'menunggu_perbaikan' ? 'badge-rusak' : 'badge-ok' }}">
                {{ $pemakai->aset->status === 'menunggu_perbaikan' ? 'Dikembalikan Dalam Kondisi Rusak' : 'Dikembalikan Dalam Kondisi Baik' }}
            </span>
        </div>

        @if ($pemakai->catatan_kembali)
            <div style="margin-top: 12px;">
                <div class="catatan-label">Catatan</div>
                <div>{{ $pemakai->catatan_kembali }}</div>
            </div>
        @endif
